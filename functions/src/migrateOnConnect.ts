...
export const migrateAnonToWallet = functions
  .region(region)
  .https.onCall(async (data: { anonUid?: string; walletAddress?: string }) => {
    const anonUid: string | undefined = data?.anonUid;
    const walletAddressRaw: string | undefined = data?.walletAddress;
